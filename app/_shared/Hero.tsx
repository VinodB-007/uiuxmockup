...
function Hero() {
  ...
  const [device, setDevice] = useState<"Website" | "Mobile">("Website");
  ...
  return (
    <div className="p-10 md:px-24 lg:px-48 xl:px-60 mt-8">
      <div className="relative flex items-center justify-center w-full mb-5">
        <div className="group relative max-w-sm flex items-center justify-center rounded-full px-4 py-1.5 shadow-[inset_0_-8px_10px_#8fdfff1f] transition-shadow duration-500 ease-out hover:shadow-[inset_0_-5px_10px_#8fdfff3f]"></div>
        ...
        <InputGroup className="max-w-xl bg-white z-10 rounded-2xl">
          ...
          <InputGroupAddon align="block-end">
            <Select
              ...
              onValueChange={(value) =>
                setDevice(value as "Website" | "Mobile")
              }
            >
      ...
      <div className="flex gap-3 mt-4 cursor-pointer flex-wrap justify-center">
        {suggestions.map((suggestion, index) => (
          <div
            key={index}
            onClick={() => setUserInput(suggestion.description)}
            className="p-2 border rounded-2xl flex flex-col items-center bg-white z-10 hover:shadow-md transition"
          >
